...
function mealPeople(date,slot){return Number(mealItems(date,slot)[0]?.people)||4}
function setMealPeople(date,slot,value){const p=Math.max(1,Number(value)||1);mealItems(date,slot).forEach(x=>x.people=p);savePlanData();renderPlan();}
function savePlanData(){localStorage.setItem(planStore,JSON.stringify({version:2,weekStart:currentWeekStart,time:$('#planTime')?.value||'',items:plan.map(x=>({uid:x.uid||uid(),date:x.date,slot:x.slot,id:x.recipe.id,people:Number(x.people)||4,role:x.role||'',notes:x.notes||''}))}));markDirty();}
...
function defaultDaySlots(){return Object.fromEntries(days.map((_,i)=>[i,{Matin:false,Midi:false,Soir:false}]))}
function readDaySlots(){try{return{...defaultDaySlots(),...JSON.parse(localStorage.getItem(slotStore)||'{}')}}catch{return defaultDaySlots()}}
...
function saveDaySlots(){const state=defaultDaySlots();$$('[data-day-slot]').forEach(c=>state[c.dataset.daySlot][c.dataset.slot]=c.checked);localStorage.setItem(slotStore,JSON.stringify(state));}
...
$('#prevWeek').onclick=()=>{currentWeekStart=addDaysISO(currentWeekStart,-7);localStorage.setItem(weekStore,currentWeekStart);renderPlan()};
$('#nextWeek').onclick=()=>{currentWeekStart=addDaysISO(currentWeekStart,7);localStorage.setItem(weekStore,currentWeekStart);renderPlan()};
$('#currentWeek').onclick=()=>{currentWeekStart=mondayISO(new Date());localStorage.setItem(weekStore,currentWeekStart);renderPlan()};
...
function transferMealToShopping(date,slot){const items=mealItems(date,slot);if(!items.length)return alert('Ajoute d’abord une recette à ce repas.');const key=mealKey(date,slot),transfers=mealTransfers(),previous=transfers[key]||[];
...
function loadSaved(){try{const s=JSON.parse(localStorage.getItem(planStore)||localStorage.getItem('hg-plan-v26')||'null');if(s){$('#planTime').value=s.time||'';currentWeekStart=s.weekStart||localStorage.getItem(weekStore)||mondayISO(new Date());plan=(s.items||[]).map(x=>{const date=x.date||addDaysISO(currentWeekStart,Number(x.dayIndex)||0);return{uid:x.uid||uid(),date,slot:x.slot,people:Number(x.people)||Number(s.people)||4,role:x.role||'',notes:x.notes||'',recipe:recipes.find(r=>r.id===x.id)}}).filter(x=>x.recipe)}shopping=(JSON.parse(localStorage.getItem(shoppingStore)||localStorage.getItem('hg-shopping-v26')||localStorage.getItem('hg-shopping')||'[]')||[]).map(normalizeShoppingItem)}catch(e){console.error(e)}localStorage.setItem(weekStore,currentWeekStart);renderPlan();renderShopping()}
