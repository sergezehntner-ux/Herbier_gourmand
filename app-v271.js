...
$('#people').onchange=()=>{localStorage.setItem('hg-people',$('#people').value);renderPlan();invalidateShopping('Le nombre de personnes a changé : recrée les courses.');};$('#clearPlan').onclick=()=>{if(confirm('Vider le planning et remettre tous les paramètres à zéro ?')){plan=[];localStorage.removeItem(planStore);$('#people').value=0;$('#planTime').value='';localStorage.setItem('hg-people','0');$$('[data-day-slot]').forEach(c=>{c.checked=false;c.parentElement.classList.remove('selected')});saveDaySlots();updateSlotStatus();invalidateShopping('Planning et paramètres remis à zéro.');renderPlan()}};$('#savePlan').onclick=()=>{localStorage.setItem(planStore,JSON.stringify({people:+$('#people').value,time:$('#planTime').value,items:plan.map(x=>({dayIndex:x.dayIndex,slot:x.slot,id:x.recipe.id}))}));$('#planFreshness').textContent='Planning enregistré sur cet appareil.';$('#planFreshness').classList.remove('hidden');};
