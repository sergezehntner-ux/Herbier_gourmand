...
async function init(){
  ...
  if('serviceWorker' in navigator) navigator.serviceWorker.register('sw.js?v=2940', {updateViaCache:'none'});
...
function shoppingRow(x){const amount=x.qty?`${Math.round(Number(x.qty)*100)/100} ${esc(x.unit)}`:x.text?`${esc(x.text)} ${esc(x.unit)}`:'';const aisle=x.aisle||'Rayon à définir';return `<div class="shop-item ${x.checked?'checked':''}" data-shopping-id="${esc(x.id)}"><input type="checkbox" data-shop-check="${esc(x.id)}" ${x.checked?'checked':''}><button class="shop-text" data-edit-shopping="${esc(x.id)}"><span class="shop-main"><strong>${esc(x.name)}</strong>${amount?` <span>— ${amount}</span>`:''}</span><span class="shop-aisle-badge">${esc(aisle)}</span></button></div>`}
