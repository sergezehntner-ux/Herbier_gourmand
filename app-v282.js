...
function savePlanData(){localStorage.setItem(planStore,JSON.stringify({version:2,weekStart:currentWeekStart,time:$('#planTime')?.value||'',items:plan.map(x=>({uid:x.uid||uid(),date:x.date,slot:x.slot,id:x.recipe.id,people:(Number(x.people)>=1&&Number(x.people)<=10?Number(x.people):null),role:x.role||'',notes:x.notes||''}))}));markDirty();}
...
function renderPlan(){
  ...
  const activeDates=weekDates.filter(date=>selected.some(x=>x.date===date)||plan.some(x=>x.date===date));
  if(!activeDates.length){$('#weekPlan').innerHTML='<p class="muted">Aucun repas sélectionné pour cette semaine.</p>';return}
  $('#weekPlan').innerHTML=activeDates.map(date=>{const d=new Date(`${date}T12:00:00`);const di=(d.getDay()+6)%7;const visibleSlots=[...new Set([...selected.filter(x=>x.date===date).map(x=>x.slot),...plan.filter(x=>x.date===date).map(x=>x.slot)])].sort((a,b)=>slots.indexOf(a)-slots.indexOf(b));return `<article class="day"><h3>${esc(dateLabel(date))}</h3>${visibleSlots.map(slot=>{const matches=mealItems(date,slot),p=mealPeople(date,slot);return `<section class="meal-group"><div class="meal-group-head"><strong>${slot}</strong><label class="meal-people">Personnes <select data-meal-people="${date}|${slot}"><option value="">—</option>${Array.from({length:10},(_,i)=>i+1).map(n=>`<option value="${n}" ${p===n?'selected':''}>${n}</option>`).join('')}</select></label><button data-add-plan="${date}|${slot}">+ Ajouter une recette</button><button class="primary" data-transfer-meal="${date}|${slot}" ${matches.length?'':'disabled'}>Transférer dans les courses</button></div>${matches.length?matches.map(m=>{const i=plan.indexOf(m),r=m.recipe;return `<div class="meal meal-line"><button class="meal-title-link" data-show-plan="${i}"><span>${m.role?`<b>${esc(m.role)}</b> · `:''}${esc(r.title)}</span><small>${r.time||'?'} min · ${esc(r.category)}${m.people?` · adapté à ${m.people} pers.`:' · personnes à définir'}</small></button><div class="meal-actions"><button data-role-plan="${i}">Rôle</button><button data-change-plan="${i}">Changer</button><button data-move-plan="${i}">Déplacer</button><button data-remove-plan="${i}" class="danger-soft">Retirer</button></div></div>`}).join(''):`<div class="meal meal-empty"><span class="meta">Aucune recette choisie</span></div>`}</section>`}).join('')}</article>`}).join('');
...
function producerTerms(p){return [...new Set([...(p.products||[]),...(p.tags||[])].map(x=>String(x||'').trim()).filter(Boolean))]}
function producerValues(field){return [...new Set(producers.flatMap(p=>field==='tags'?producerTerms(p):[p[field]]).map(x=>String(x||'').trim()).filter(Boolean))].sort((a,b)=>a.localeCompare(b,'fr',{sensitivity:'base'}))}
