...
function savePlanData(){localStorage.setItem(planStore,JSON.stringify({version:3,weekStart:currentWeekStart,time:$('#planTime')?.value||'',items:plan.map(x=>({uid:x.uid||uid(),date:x.date,slot:x.slot,id:x.recipe.id,people:(Number(x.people)>=1&&Number(x.people)<=10?Number(x.people):null),role:x.role||'',notes:x.notes||'',preparePreviousDay:Boolean(x.preparePreviousDay)}))}));markDirty();}
...
function openMainView(id){if(id==='planner'){currentWeekStart=mondayISO(new Date());localStorage.setItem(weekStore,currentWeekStart);renderDaySlotChoices();renderPlan()}switchView(id)}
$$('nav button').forEach(b=>b.onclick=()=>{selectionContext=null;updateSelectionBar();openMainView(b.dataset.view)});
...
function fillCategories(){const cats=[...new Set(recipes.map(r=>r.category).filter(Boolean))].sort((a,b)=>a.localeCompare(b,'fr',{sensitivity:'base'}));const filter=$('#category'),filterCurrent=filter?.value||'';filter.innerHTML='<option value="">Toutes catégories</option>'+cats.map(c=>`<option value="${esc(c)}">${esc(c)}</option>`).join('');if(cats.includes(filterCurrent))filter.value=filterCurrent;const form=$('#recipeCategory'),current=form?.value||'';if(form){form.innerHTML='<option value="">Choisir…</option>'+cats.map(c=>`<option value="${esc(c)}">${esc(c)}</option>`).join('')+'<option value="__other__">Autre…</option>';if(cats.includes(current))form.value=current;}}
let recipeComplementDraft=[];
function recipeById(id){return recipes.find(r=>r.id===String(id))}
function leftoverPlans(){try{return JSON.parse(localStorage.getItem(leftoverPlanStore)||'[]')||[]}catch{return[]}}
function saveLeftoverPlans(items){localStorage.setItem(leftoverPlanStore,JSON.stringify(items));markDirty()}
function leftoverIdeas(r){return String(r?.leftoverIdeas||'').split(/\r?\n/).map(x=>x.trim()).filter(Boolean)}
function useRecipeLeftovers(recipeId){const r=recipeById(recipeId);if(!r)return;const ideas=leftoverIdeas(r);if(!ideas.length)return alert('Ajoute d’abord une ou plusieurs idées dans « Utiliser des restes ».');const cooked=[...(r.cookedDates||[])].sort();if(!cooked.length)return alert('Cette recette n’a pas encore été marquée comme cuisinée.');let idea=ideas[0];if(ideas.length>1){const answer=prompt('Quel reste veux-tu planifier ?\n'+ideas.map((x,i)=>`${i+1}. ${x}`).join('\n'),'1');if(answer===null)return;const n=Number(answer);if(!Number.isInteger(n)||n<1||n>ideas.length)return alert('Choix invalide.');idea=ideas[n-1]}let sourceDate=cooked[cooked.length-1];if(cooked.length>1){const answer=prompt('De quelle préparation proviennent ces restes ?\n'+cooked.map((x,i)=>`${i+1}. ${dateLabel(x)}`).join('\n'),String(cooked.length));if(answer===null)return;const n=Number(answer);if(!Number.isInteger(n)||n<1||n>cooked.length)return alert('Choix invalide.');sourceDate=cooked[n-1]}const targetDate=prompt('À quelle date veux-tu utiliser ces restes ? (AAAA-MM-JJ)',addDaysISO(sourceDate,1));if(!targetDate)return;if(targetDate<=sourceDate)return alert('Choisis une date ultérieure à la date de cuisson.');let slot=prompt('Pour quel repas ? Matin, Midi ou Soir','Midi');if(slot===null)return;slot=slot.trim().toLowerCase();slot=slot==='matin'?'Matin':slot==='midi'?'Midi':slot==='soir'?'Soir':'';if(!slot)return alert('Repas invalide.');const matches=mealItems(targetDate,slot);let mode='seul',replacedTitle='';if(matches.length){const answer=prompt('Un repas est déjà planifié.\nR = remplacer une recette planifiée\nC = utiliser les restes comme complément\nA = annuler','C');if(answer===null||answer.trim().toUpperCase()==='A')return;if(answer.trim().toUpperCase()==='R'){mode='remplacement';let target=matches[0];if(matches.length>1){const pick=prompt('Quelle recette remplacer ?\n'+matches.map((x,i)=>`${i+1}. ${x.recipe.title}`).join('\n'),'1');if(pick===null)return;const n=Number(pick);if(!Number.isInteger(n)||n<1||n>matches.length)return alert('Choix invalide.');target=matches[n-1]}replacedTitle=target.recipe.title;const idx=plan.indexOf(target);if(idx>=0)plan.splice(idx,1);savePlanData()}else if(answer.trim().toUpperCase()==='C')mode='complement';else return alert('Choix invalide.')}const items=leftoverPlans();items.push({id:uid(),recipeId:r.id,sourceTitle:r.title,sourceDate,text:idea,date:targetDate,slot,mode,replacedTitle});saveLeftoverPlans(items);currentWeekStart=mondayISO(new Date(`${targetDate}T12:00:00`));localStorage.setItem(weekStore,currentWeekStart);renderDaySlotChoices();renderPlan();alert(`Restes planifiés pour ${dateLabel(targetDate)} · ${slot}.`)}
function removeLeftoverPlan(id){const items=leftoverPlans(),x=items.find(v=>v.id===id);if(!x)return;if(confirm(`Retirer « ${x.text} » du planning ?`)){saveLeftoverPlans(items.filter(v=>v.id!==id));renderPlan()}}
function leftoverMarkupForMeal(date,slot){const items=leftoverPlans().filter(x=>x.date===date&&x.slot===slot);return items.map(x=>`<div class="meal meal-line leftover-plan"><button class="meal-title-link" data-leftover-source="${esc(x.recipeId)}"><span><b>${x.mode==='complement'?'Complément · restes':'Restes'}</b> · ${esc(x.text)}</span><small>de ${esc(x.sourceTitle)} cuisiné le ${esc(dateLabel(x.sourceDate))}${x.replacedTitle?` · remplace ${esc(x.replacedTitle)}`:''}</small></button><div class="meal-actions"><button data-remove-leftover="${esc(x.id)}" class="danger-soft">Retirer</button></div></div>`).join('')}
...
function recipeCard(r){const choose=selectionContext?`<button class="primary" data-choose="${esc(r.id)}">Choisir pour ${esc(dateLabel(selectionContext.date))} ${esc(selectionContext.slot.toLowerCase())}</button>`:'';return `<article class="recipe" data-recipe-open="${esc(r.id)}"><div class="recipe-head"><div class="meta">${esc(r.category)} · ${r.time||'?'} min · ${r.servings} pers.</div><h3>${esc(r.title)}</h3><div class="badges">${(r.tags||[]).slice(0,5).map(t=>`<span>${esc(t)}</span>`).join('')}</div></div><div class="recipe-actions">${choose}<button data-edit="${esc(r.id)}">Modifier</button><button data-print-recipe="${esc(r.id)}">Imprimer</button></div></article>`;}
...
function chooseRecipeForPlan(id){if(!selectionContext)return;const r=recipes.find(x=>x.id===id);if(!r)return;const ctx={...selectionContext};if(Number.isInteger(ctx.index)&&plan[ctx.index])plan[ctx.index].recipe=r;else plan.push({uid:uid(),date:ctx.date,slot:ctx.slot,recipe:r,people:mealPeople(ctx.date,ctx.slot)||null,role:''});selectionContext=null;savePlanData();renderPlan();updateSelectionBar();switchView('planner');}
...
function showRecipe(id,from=''){const r=recipes.find(x=>x.id===id);if(!r)return;if(from){rememberScroll(from);previousView=from}else if(!previousView)previousView='recipes';viewedRecipeId=id;$('#recipeViewContent').innerHTML=`<div class="recipe-title"><div class="meta">${esc(r.category)} · ${r.time||'?'} min · ${r.servings} pers. · ${esc(r.temperature)}</div><h2>${esc(r.title)}</h2>${recipeSourceMarkup(r.source)}</div><div class="recipe-columns"><section><h3>Ingrédients</h3><ul>${scaledIngredients(r,r.servings).map(x=>`<li>${esc(x)}</li>`).join('')}</ul></section><section><h3>Préparation</h3><ol>${r.steps.map(x=>`<li>${esc(x)}</li>`).join('')}</ol>${r.notes?`<h3>Notes</h3><p>${esc(r.notes).replace(/\n/g,'<br>')}</p>`:''}${r.leftoverIdeas?`<h3>Utiliser des restes</h3><p>${esc(r.leftoverIdeas).replace(/\n/g,'<br>')}</p>`:''}</section></div>${recipeComplementMarkup(r)}${r.leftoverIdeas?`<section class="recipe-memory-block"><h3>Utiliser des restes</h3><p>${esc(r.leftoverIdeas).replace(/\n/g,'<br>')}</p><button class="primary" data-use-leftovers="${esc(r.id)}">Planifier l’utilisation des restes</button></section>`:''}${recipeMemoryMarkup(r)}`;switchView('recipeView');bindRecipeDetailLinks();$$('[data-use-leftovers]').forEach(b=>b.onclick=()=>useRecipeLeftovers(b.dataset.useLeftovers));}
...
function openRecipe(id){const r=recipes.find(x=>x.id===id);$('#recipeDialogTitle').textContent=r?'Modifier la recette':'Nouvelle recette';$('#recipeId').value=r?.id||'';$('#recipeTitle').value=r?.title||'';$('#recipeCategory').value=r?.category||'';$('#recipeTime').value=r?.time||30;$('#recipeServings').value=r?.servings||4;$('#recipeTemperature').value=r?.temperature||'chaud';$('#recipeMeat').checked=!!r?.avecViande;$('#recipeIngredients').value=(r?.ingredients||[]).map(i=>i.join(' / ')).join('\n');$('#recipeSteps').value=(r?.steps||[]).join('\n');$('#recipeTags').value=(r?.tags||[]).join(', ');$('#recipeComplementText').value=r?.complementText||'';$('#recipeLeftovers').value=r?.leftoverIdeas||'';recipeComplementDraft=[...(r?.complementRecipeIds||[])];fillComplementRecipeSelect(r?.id||'');renderComplementDraft();$('#deleteRecipe').classList.toggle('hidden',!r);$('#duplicateRecipe').classList.toggle('hidden',!r);$('#recipeDialog').showModal();}
$('#newRecipe').onclick=()=>openRecipe();$('#closeRecipe').onclick=()=>$('#recipeDialog').close();$('#addRecipeComplement').onclick=()=>{const id=$('#recipeComplementSelect').value;if(id&&!recipeComplementDraft.includes(id)){recipeComplementDraft.push(id);renderComplementDraft()}$('#recipeComplementSelect').value=''};
...
function formRecipe(newId=false){const ingredients=$('#recipeIngredients').value.split('\n').map(x=>x.trim()).filter(Boolean).map(line=>{const [n,q='',u='']=line.split('/').map(x=>x.trim());return[n,parseNumber(q),u]});const existing=recipes.find(x=>x.id===$('#recipeId').value);return normalizeRecipe({id:newId||$('#recipeId').value||slug($('#recipeTitle').value),title:$('#recipeTitle').value.trim(),category:$('#recipeCategory').value.trim(),time:+$('#recipeTime').value,servings:+$('#recipeServings').value,temperature:$('#recipeTemperature').value,avecViande:$('#recipeMeat').checked,ingredients,steps:$('#recipeSteps').value.split('\n').map(x=>x.trim()).filter(Boolean),tags:$('#recipeTags').value.split(',').map(x=>x.trim()).filter(Boolean),source:existing?.source||'',notes:existing?.notes||'',paprikaUid:existing?.paprikaUid||'',complementRecipeIds:[...recipeComplementDraft],complementText:$('#recipeComplementText').value.trim(),leftoverIdeas:$('#recipeLeftovers').value.trim(),cookedDates:existing?.cookedDates||[],favorite:existing?.favorite||false,cookingComments:existing?.cookingComments||[]});}
$('#recipeForm').onsubmit=e=>{e.preventDefault();const r=formRecipe();const i=recipes.findIndex(x=>x.id===r.id);if(i>=0)recipes[i]=r;else recipes.unshift(r);plan.forEach(item=>{if(item.recipe?.id===r.id)item.recipe=r});saveRecipes();savePlanData();fillCategories();keepScroll(renderRecipes,'recipes');renderPlan();invalidateShopping('Une recette a été modifiée : les affichages du planning sont à jour. Retransfère le repas dans les courses si les ingrédients ont changé.');$('#recipeDialog').close();if(viewedRecipeId===r.id){const pi=plan.findIndex(x=>x.uid===viewedPlanItemId);if(previousView==='planner'&&pi>=0)showPlanRecipe(pi);else showRecipe(r.id)}};
...
function saveDaySlots(){const state=defaultDaySlots();$$('[data-day-slot]').forEach(c=>state[c.dataset.daySlot][c.dataset.slot]=c.checked);const map=readWeekSlots();map[currentWeekStart]=state;localStorage.setItem(weekSlotStore,JSON.stringify(map));markDirty();}
...
function choices(exclude=[]){const max=+$('#planTime').value||999;let pool=recipes.filter(r=>(!r.time||r.time<=max)&&!exclude.includes(r.id));return pool.length?pool:recipes.filter(r=>!exclude.includes(r.id));}
function pickRandom(a){return a[Math.floor(Math.random()*a.length)]}
$('#generatePlan').onclick=()=>{const targets=selectedDaySlots();if(!targets.length)return alert('Choisis au moins un repas.');const used=[];targets.forEach(t=>{if(mealItems(t.date,t.slot).length)return;let pool=choices(used);if(!pool.length)pool=choices([]);const recipe=pickRandom(pool);if(recipe){used.push(recipe.id);plan.push({uid:uid(),date:t.date,slot:t.slot,recipe,people:null,role:''})}});savePlanData();renderPlan();};
...
function setMealNote(date,slot,value){const all=mealNotes(),key=mealKey(date,slot),v=String(value||'').trim();if(v)all[key]=v;else delete all[key];localStorage.setItem(mealNoteStore,JSON.stringify(all));markDirty()}
...
function showPlanRecipe(index){const m=plan[index];if(!m)return;viewedPlanItemId=m.uid;rememberScroll('planner');viewedRecipeId=m.recipe.id;previousView='planner';const rows=scaledIngredientRows(m.recipe,m.people,m.qtyOverrides||{});$('#recipeViewContent').innerHTML=`<div class="recipe-title"><div class="meta">${esc(m.recipe.category)} · ${m.recipe.time||'?'} min${m.people?` · adapté à ${m.people} pers.`:' · portions originales'} · ${esc(dateLabel(m.date))} ${esc(m.slot.toLowerCase())}</div><h2>${esc(m.recipe.title)}</h2>${recipeSourceMarkup(m.recipe.source)}</div><div class="notice subtle">Cette adaptation appartient uniquement à ce repas. La recette originale reste inchangée.</div><div class="recipe-columns"><section><h3>Ingrédients</h3><ul>${rows.map(x=>`<li>${esc(x.text)}</li>`).join('')}</ul></section><section><h3>Préparation</h3><ol>${m.recipe.steps.map(x=>`<li>${esc(x)}</li>`).join('')}</ol>${m.notes?`<h3>Notes du repas</h3><p>${esc(m.notes).replace(/\n/g,'<br>')}</p>`:''}${m.recipe.leftoverIdeas?`<h3>Utiliser des restes</h3><p>${esc(m.recipe.leftoverIdeas).replace(/\n/g,'<br>')}</p>`:''}</section></div>${recipeComplementMarkup(m.recipe)}${m.recipe.leftoverIdeas?`<section class="recipe-memory-block"><h3>Utiliser des restes</h3><p>${esc(m.recipe.leftoverIdeas).replace(/\n/g,'<br>')}</p><button class="primary" data-use-leftovers="${esc(m.recipe.id)}">Planifier l’utilisation des restes</button></section>`:''}${recipeMemoryMarkup(m.recipe)}`;switchView('recipeView');bindRecipeDetailLinks();$$('[data-use-leftovers]').forEach(b=>b.onclick=()=>useRecipeLeftovers(b.dataset.useLeftovers));}
function renderPlan(){
  ...
  const activeDates=weekDates.filter(date=>selected.some(x=>x.date===date)||plan.some(x=>x.date===date)||leftoverPlans().some(x=>x.date===date)||preparationsForDate(date).length);
  ...
  $('#weekPlan').innerHTML=activeDates.map(date=>{
    ...
    const visibleSlots=[...new Set([...selected.filter(x=>x.date===date).map(x=>x.slot),...plan.filter(x=>x.date===date).map(x=>x.slot),...leftoverPlans().filter(x=>x.date===date).map(x=>x.slot)])].sort((a,b)=>slots.indexOf(a)-slots.indexOf(b));
  ...
  $$('[data-leftover-source]').forEach(b=>b.onclick=()=>showRecipe(b.dataset.leftoverSource,'planner'));
  $$('[data-remove-leftover]').forEach(b=>b.onclick=()=>removeLeftoverPlan(b.dataset.removeLeftover));
...
function normalizeShoppingItem(x={}){const pref=shoppingAssignments()[norm(x.name||x.ingredient)]||{};return{id:x.id||uid(),name:x.name||x.ingredient||'',qty:x.qty??x.quantite??0,text:x.text||'',unit:x.unit||x.unite||'',store:x.store||x.magasin||pref.store||'',aisle:x.aisle||x.rayon||pref.aisle||'',checked:Boolean(x.checked??x.coche),manual:Boolean(x.manual),origins:Array.isArray(x.origins)?x.origins:Array.isArray(x.sources)?x.sources:Array.isArray(x.origine)?x.origine:[],originRefs:Array.isArray(x.originRefs)?x.originRefs:[]}}
...
function consolidateShopping(){const out=[];shopping.map(normalizeShoppingItem).forEach(x=>{if(x.manual){out.push(x);return}const hit=out.find(y=>!y.manual&&norm(y.name)===norm(x.name)&&norm(y.unit)===norm(x.unit));if(!hit){out.push(x);return}hit.qty=Math.round((Number(hit.qty||0)+Number(x.qty||0))*100)/100;hit.origins=[...new Set([...(hit.origins||[]),...(x.origins||[])])];hit.originRefs=[...(hit.originRefs||[]),...(x.originRefs||[])].filter((v,i,a)=>a.findIndex(z=>z.recipeId===v.recipeId&&z.date===v.date&&z.slot===v.slot)===i);if(!hit.store&&x.store)hit.store=x.store;if(!hit.aisle&&x.aisle)hit.aisle=x.aisle});shopping=out}
function saveShopping(){shopping=shopping.map(normalizeShoppingItem);consolidateShopping();localStorage.setItem(shoppingStore,JSON.stringify(shopping));rememberShoppingAssignments();markDirty()}
function transferMealToShopping(date,slot){const items=mealItems(date,slot);if(!items.length)return alert('Ajoute d’abord une recette à ce repas.');if(!mealPeople(date,slot))return alert('Indique d’abord le nombre de personnes pour ce repas.');const key=mealKey(date,slot),transfers=mealTransfers(),previous=transfers[key]||[];
  previous.forEach(old=>{const x=shopping.find(s=>norm(s.name)===norm(old.name)&&norm(s.unit)===norm(old.unit));if(x&&typeof x.qty==='number')x.qty=Math.max(0,Math.round((x.qty-Number(old.qty||0))*100)/100)});
  shopping=shopping.filter(x=>x.manual||Number(x.qty)>0||x.text);
  const added=[];const prefs=shoppingAssignments();
  items.forEach(m=>{const rows=scaledIngredientRows(m.recipe,m.people,m.qtyOverrides||{});const freeComplements=String(m.recipe.complementText||'').split(/\r?\n/).map(x=>x.trim()).filter(Boolean).map(parseComplementShoppingLine);[...rows,...freeComplements].forEach(row=>{const q=Number(row.qty);if(!Number.isFinite(q)||q===0)return;let x=shopping.find(s=>!s.manual&&norm(s.name)===norm(row.name)&&norm(s.unit)===norm(row.unit));const pref=prefs[norm(row.name)]||{};if(!x){x=normalizeShoppingItem({name:row.name,qty:0,unit:row.unit,store:pref.store||'',aisle:pref.aisle||'',origins:[],originRefs:[]});shopping.push(x)}x.qty=Math.round((Number(x.qty||0)+q)*100)/100;const origin=`${dateLabel(date)} ${slot} · ${m.recipe.title}`;if(!x.origins.includes(origin))x.origins.push(origin);x.originRefs=x.originRefs||[];if(!x.originRefs.some(o=>o.recipeId===m.recipe.id&&o.date===date&&o.slot===slot))x.originRefs.push({recipeId:m.recipe.id,title:m.recipe.title,date,slot});added.push({name:row.name,unit:row.unit,qty:q})})});
  transfers[key]=added;saveMealTransfers(transfers);saveShopping();renderShopping();$('#planFreshness').textContent=`${dateLabel(date)} ${slot.toLowerCase()} transféré dans la liste des courses.`;$('#planFreshness').classList.remove('hidden');switchView('shopping');
}
...
function printRecipe(r,people=null){if(!r)return;const p=people||r.servings;const body=`<h1>${esc(r.title)}</h1><p>${r.time||'?'} min · ${p} personnes · ${esc(r.category)}</p><div class="columns"><section><h2>Ingrédients</h2><ul>${scaledIngredients(r,p).map(x=>`<li>${esc(x)}</li>`).join('')}</ul></section><section><h2>Préparation</h2><ol>${r.steps.map(x=>`<li>${esc(x)}</li>`).join('')}</ol></section></div>`;printDocument(r.title,body)}
...
function loadSaved(){try{const s=JSON.parse(localStorage.getItem(planStore)||localStorage.getItem('hg-plan-v26')||'null');if(s){$('#planTime').value=s.time||'';currentWeekStart=s.weekStart||localStorage.getItem(weekStore)||mondayISO(new Date());plan=(s.items||[]).map(x=>{const date=x.date||addDaysISO(currentWeekStart,Number(x.dayIndex)||0);return{uid:x.uid||uid(),date,slot:x.slot,people:(Number(x.people)>=1&&Number(x.people)<=10?Number(x.people):(Number(s.people)>=1&&Number(s.people)<=10?Number(s.people):null)),role:x.role||'',notes:x.notes||'',preparePreviousDay:Boolean(x.preparePreviousDay),recipe:recipes.find(r=>r.id===x.id)}}).filter(x=>x.recipe)}shopping=(JSON.parse(localStorage.getItem(shoppingStore)||localStorage.getItem('hg-shopping-v26')||localStorage.getItem('hg-shopping')||'[]')||[]).map(normalizeShoppingItem)}catch(e){console.error(e)}localStorage.setItem(weekStore,currentWeekStart);renderDaySlotChoices();renderPlan();renderShopping()}
...
function openHerbEditor(id=viewedHerbId){
 if(READONLY)return;
 const existing=herbs.find(x=>x.id===id),h=existing||normalizeHerb({id:'',name:''}),isNew=!existing;
 ...
 const map={Name:'name',Family:'family',Origin:'origin',Season:'season',Intensity:'intensity',Forms:'forms',Flavor:'flavor',Uses:'uses',IdealFoods:'idealFoods',Pairings:'recommendedPairings',Cooking:'cookingBehavior',Preparation:'preparation',Quantity:'quantity',Substitution:'substitution',Avoid:'avoid',Conservation:'conservation',Where:'whereToBuy',RecipeIdeas:'recipeIdeas',ChefTip:'chefTip'};
