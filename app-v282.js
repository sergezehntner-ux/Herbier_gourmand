...
function recipeDifficultyLabel(v){return ({facile:'Facile',moyenne:'Moyenne',difficile:'Difficile',expert:'Expert'})[v]||'Difficulté non définie'}
...
function recipeCard(r){const choose=selectionContext?`<button class="primary" data-choose="${esc(r.id)}">Choisir pour ${esc(dateLabel(selectionContext.date))} ${esc(selectionContext.slot.toLowerCase())}</button>`:'';return `<article class="recipe" data-recipe-open="${esc(r.id)}"><div class="recipe-head"><div class="meta">${esc(r.category)} · ${esc(recipeTypeLabel(r.type))} · ${r.servings} pers.</div><h3>${esc(r.title)}</h3><div class="badges">${(r.tags||[]).slice(0,5).map(t=>`<span>${esc(t)}</span>`).join('')}</div></div><div class="recipe-actions">${choose}<button data-edit="${esc(r.id)}">Modifier</button><button data-print-recipe="${esc(r.id)}">Imprimer</button></div></article>`;}
function renderRecipes(){const q=norm($('#search').value),cat=$('#category').value;const found=recipes.filter(r=>(!cat||r.category===cat)&&(!q||norm(JSON.stringify(r)).includes(q))).sort((a,b)=>a.title.localeCompare(b.title,'fr',{sensitivity:'base'}));$('#recipeCount').textContent=`${found.length} recette${found.length>1?'s':''}`;$('#recipeList').innerHTML=found.map(recipeCard).join('')||'<p>Aucune recette trouvée.</p>';bindRecipeCards($('#recipeList'));}
...
$('#search').oninput=renderRecipes;$('#category').onchange=renderRecipes;
...
function showRecipe(id,from=''){const r=recipes.find(x=>x.id===id);if(!r)return;if(from){rememberScroll(from);previousView=from}else if(!previousView)previousView='recipes';viewedRecipeId=id;$('#recipeViewContent').innerHTML=`<div class="recipe-title"><div class="meta">${esc(r.category)} · ${esc(recipeTypeLabel(r.type))} · ${esc(recipeEffortLabel(r.effort))} · ${esc(recipeDifficultyLabel(r.difficulty))} · ${r.servings} pers. · ${esc(r.temperature)}</div><h2>${esc(r.title)}</h2>${recipeSourceMarkup(r.source)}</div><div class="recipe-columns"><section><h3>Ingrédients</h3><ul>${scaledIngredients(r,r.servings).map(x=>`<li>${esc(x)}</li>`).join('')}</ul></section><section><h3>Préparation</h3><ol>${r.steps.map(x=>`<li>${esc(x)}</li>`).join('')}</ol>${r.notes?`<h3>Notes</h3><p>${esc(r.notes).replace(/\n/g,'<br>')}</p>`:''}</section></div>${recipeMemoryMarkup(r)}`;switchView('recipeView');bindRecipeDetailLinks();}
...
function formRecipe(newId=false){const ingredients=$('#recipeIngredients').value.split('\n').map(x=>x.trim()).filter(Boolean).map(line=>{const [n,q='',u='']=line.split('/').map(x=>x.trim());return[n,parseNumber(q),u]});const existing=recipes.find(x=>x.id===$('#recipeId').value);return normalizeRecipe({id:newId||$('#recipeId').value||slug($('#recipeTitle').value),title:$('#recipeTitle').value.trim(),category:$('#recipeCategory').value.trim(),time:existing?.time||0,servings:+$('#recipeServings').value,temperature:$('#recipeTemperature').value,avecViande:$('#recipeType').value==='viande',type:$('#recipeType').value,effort:$('#recipeEffort').value,difficulty:$('#recipeDifficulty').value,ingredients,steps:$('#recipeSteps').value.split('\n').map(x=>x.trim()).filter(Boolean),tags:$('#recipeTags').value.split(',').map(x=>x.trim()).filter(Boolean),source:existing?.source||'',notes:existing?.notes||'',paprikaUid:existing?.paprikaUid||'',complementRecipeIds:existing?.complementRecipeIds||[],complementText:existing?.complementText||'',leftoverIdeas:existing?.leftoverIdeas||'',cookedDates:existing?.cookedDates||[],favorite:existing?.favorite||false,cookingComments:existing?.cookingComments||[]});}
...
function choices(exclude=[]){const pool=recipes.filter(r=>!exclude.includes(r.id));return pool.length?pool:recipes;}
...
function sameShoppingArticle(a,b){return norm(a?.name)===norm(b?.name)&&norm(a?.unit)===norm(b?.unit)}
...
function printRecipe(r,people=null){if(!r)return;const p=people||r.servings;const body=`<h1>${esc(r.title)}</h1><p>${p} personnes · ${esc(r.category)} · ${esc(recipeTypeLabel(r.type))} · ${esc(recipeEffortLabel(r.effort))} · ${esc(recipeDifficultyLabel(r.difficulty))}</p><div class="columns"><section><h2>Ingrédients</h2><ul>${scaledIngredients(r,p).map(x=>`<li>${esc(x)}</li>`).join('')}</ul></section><section><h2>Préparation</h2><ol>${r.steps.map(x=>`<li>${esc(x)}</li>`).join('')}</ol></section></div>`;printDocument(r.title,body)}
