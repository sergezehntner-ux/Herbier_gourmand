...
async function init(){
  ...
  if('serviceWorker' in navigator) navigator.serviceWorker.register('sw.js?v=29716', {updateViaCache:'none'});
...
function recipeMetaMarkup(r,portions=r.servings){return `<span><b>Catégorie :</b> ${esc(r.category||'Non définie')}</span> · <span><b>Type :</b> ${esc(recipeTypeLabel(r.type).replace(/^Type /,''))}</span> · <span><b>Effort :</b> ${esc(recipeEffortLabel(r.effort).replace(/^Effort /,''))}</span> · <span><b>Difficulté :</b> ${esc(recipeDifficultyLabel(r.difficulty))}</span> · <span><b>Température :</b> ${esc(recipeTemperatureLabel(r.temperature))}</span> · <span><b>Spécial :</b> ${esc(recipeSpecialLabel(r.special))}</span> · <span><b>Portions :</b> ${esc(portions||r.servings||'—')}</span>`}
...
function showRecipe(id,from=''){const r=recipes.find(x=>x.id===id);if(!r)return;if(from){rememberScroll(from);previousView=from}else if(!previousView)previousView='recipes';viewedRecipeId=id;$('#recipeViewContent').innerHTML=`<div class="recipe-title"><div class="meta recipe-meta-line">${recipeMetaMarkup(r)}</div><h2>${esc(r.title)}</h2><div class="badges">${(r.tags||[]).map(t=>`<span>${esc(t)}</span>`).join('')}</div>${recipeSourceMarkup(r.source)}</div><div class="recipe-columns"><section><h3>Ingrédients</h3><ul>${scaledIngredients(r,r.servings).map(x=>`<li>${esc(x)}</li>`).join('')}</ul></section><section><h3>Préparation</h3><ol>${r.steps.map(x=>`<li>${esc(x)}</li>`).join('')}</ol>${r.notes?`<h3>Notes</h3><p>${esc(r.notes).replace(/\n/g,'<br>')}</p>`:''}</section></div>${recipeMemoryMarkup(r)}`;switchView('recipeView');bindRecipeDetailLinks();}
