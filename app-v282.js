...
async function init(){
  currentWeekStart=localStorage.getItem(weekStore)||mondayISO(new Date());
  await autoLoadSharedBackup();
  migrateLegacyWeekSlots();
  renderDaySlotChoices();
  if('serviceWorker' in navigator) navigator.serviceWorker.register('sw.js?v=2931', {updateViaCache:'none'});
...
function fillCategories(){const cats=[...new Set(recipes.map(r=>r.category).filter(Boolean))].sort((a,b)=>a.localeCompare(b,'fr',{sensitivity:'base'}));$('#category').innerHTML='<option value="">Toutes catégories</option>'+cats.map(c=>`<option value="${esc(c)}">${esc(c)}</option>`).join('');const form=$('#recipeCategory'),current=form?.value||'';if(form){form.innerHTML='<option value="">Choisir…</option>'+cats.map(c=>`<option value="${esc(c)}">${esc(c)}</option>`).join('')+'<option value="__other__">Autre…</option>';if(cats.includes(current))form.value=current;}}
function recipeCard(r){const choose=selectionContext?`<button class="primary" data-choose="${esc(r.id)}">Choisir pour ${esc(dateLabel(selectionContext.date))} ${esc(selectionContext.slot.toLowerCase())}</button>`:'';return `<article class="recipe"><div class="recipe-head"><div class="meta">${esc(r.category)} · ${r.time||'?'} min · ${r.servings} pers.</div><h3>${esc(r.title)}</h3><div class="badges">${(r.tags||[]).slice(0,5).map(t=>`<span>${esc(t)}</span>`).join('')}</div></div><div class="recipe-actions">${choose}<button data-view-recipe="${esc(r.id)}">Voir la recette</button><button data-edit="${esc(r.id)}">Modifier</button><button data-print-recipe="${esc(r.id)}">Imprimer</button></div></article>`;}
...
function showRecipe(id,from='recipes'){const r=recipes.find(x=>x.id===id);if(!r)return;rememberScroll(from);viewedRecipeId=id;previousView=from;$('#recipeViewContent').innerHTML=`<div class="recipe-title"><div class="meta">${esc(r.category)} · ${r.time||'?'} min · ${r.servings} pers. · ${esc(r.temperature)}</div><h2>${esc(r.title)}</h2>${recipeSourceMarkup(r.source)}</div><div class="recipe-columns"><section><h3>Ingrédients</h3><ul>${scaledIngredients(r,r.servings).map(x=>`<li>${esc(x)}</li>`).join('')}</ul></section><section><h3>Préparation</h3><ol>${r.steps.map(x=>`<li>${esc(x)}</li>`).join('')}</ol>${r.notes?`<h3>Notes</h3><p>${esc(r.notes).replace(/\n/g,'<br>')}</p>`:''}</section></div>`;switchView('recipeView');}
...
function movePlanItem(index){const item=plan[index];if(!item)return;const dateAnswer=prompt('Nouvelle date (AAAA-MM-JJ)',item.date);if(dateAnswer===null)return;if(!/^\d{4}-\d{2}-\d{2}$/.test(dateAnswer))return alert('Date invalide.');const slotAnswer=prompt('Quel repas ? 1 Matin · 2 Midi · 3 Soir',String(slots.indexOf(item.slot)+1));if(slotAnswer===null)return;const slot=slots[Number(slotAnswer)-1];if(!slot)return alert('Repas invalide.');item.date=dateAnswer;item.slot=slot;savePlanData();keepScroll(renderPlan,'planner');}
...
function setPlanRole(index){const item=plan[index];if(!item)return;const role=prompt('Rôle dans le repas (entrée, plat, accompagnement, dessert, sauce, boisson…)',item.role||'');if(role===null)return;item.role=role.trim();savePlanData();renderPlan();}
...
function saveShopping(){shopping=shopping.map(normalizeShoppingItem);localStorage.setItem(shoppingStore,JSON.stringify(shopping));rememberShoppingAssignments();markDirty()}
function transferMealToShopping(date,slot){const items=mealItems(date,slot);if(!items.length)return alert('Ajoute d’abord une recette à ce repas.');if(!mealPeople(date,slot))return alert('Indique d’abord le nombre de personnes pour ce repas.');const key=mealKey(date,slot),transfers=mealTransfers(),previous=transfers[key]||[];
  ...
  items.forEach(m=>scaledIngredientRows(m.recipe,m.people,m.qtyOverrides||{}).forEach(row=>{const q=Number(row.qty);if(!Number.isFinite(q)||q===0)return;let x=shopping.find(s=>!s.manual&&norm(s.name)===norm(row.name)&&norm(s.unit)===norm(row.unit));const pref=prefs[norm(row.name)]||{};if(!x){x=normalizeShoppingItem({name:row.name,qty:0,unit:row.unit,store:pref.store||'',aisle:pref.aisle||'',origins:[]});shopping.push(x)}x.qty=Math.round((Number(x.qty||0)+q)*100)/100;const origin=`${dateLabel(date)} ${slot} · ${m.recipe.title}`;if(!x.origins.includes(origin))x.origins.push(origin);added.push({name:row.name,unit:row.unit,qty:q})}));
...
function renderStoreSubgroups(items){const aisles={};items.forEach(x=>(aisles[x.aisle||'Rayon à définir']??=[]).push(x));const store=items[0]?.store||'';return Object.entries(aisles).sort((a,b)=>aisleRank(store,a[0])-aisleRank(store,b[0])).map(([aisle,list])=>`<div class="shop-subgroup"><h4>${esc(aisle)}</h4>${list.map(shoppingRow).join('')}</div>`).join('')}
...
$('#producerForm').onsubmit=e=>{e.preventDefault();const p=producerFromForm(),i=producers.findIndex(x=>x.id===p.id);if(i>=0)producers[i]=p;else producers.push(p);saveProducers();renderProducerFilters();renderProducers();$('#producerDialog').close();if(viewedProducerId===p.id)showProducer(p.id,previousProducerView)};
...
function fillEditableSelect(el,values,current='',empty='Choisir…'){if(!el)return;const list=[...new Set(values.map(x=>String(x||'').trim()).filter(Boolean))].sort((a,b)=>a.localeCompare(b,'fr',{sensitivity:'base'}));if(current&&!list.includes(current))list.push(current);el.innerHTML=`<option value="">${empty}</option>`+list.map(v=>`<option value="${esc(v)}">${esc(v)}</option>`).join('')+'<option value="__other__">Autre…</option>';el.value=current||'';}
function editableSelectOther(el,label){if(el&&el.value==='__other__'){const value=prompt(`Nouveau ${label} :`);if(value?.trim()){const option=document.createElement('option');option.value=value.trim();option.textContent=value.trim();el.insertBefore(option,el.lastElementChild);el.value=value.trim()}else el.value=''}}
if($('#recipeCategory')) $('#recipeCategory').onchange=()=>editableSelectOther($('#recipeCategory'),'catégorie');
if($('#restaurantCityField')) $('#restaurantCityField').onchange=()=>editableSelectOther($('#restaurantCityField'),'lieu');
if($('#restaurantCountryField')) $('#restaurantCountryField').onchange=()=>editableSelectOther($('#restaurantCountryField'),'pays');
if($('#producerTypeField')) $('#producerTypeField').onchange=()=>editableSelectOther($('#producerTypeField'),'type');
if($('#producerRegionField')) $('#producerRegionField').onchange=()=>editableSelectOther($('#producerRegionField'),'région');
if($('#producerCityField')) $('#producerCityField').onchange=()=>editableSelectOther($('#producerCityField'),'lieu');
...
function renderRestaurantFilters(){fillRestaurantSelect('#restaurantCity',uniqueRestaurantValues('city'),'Tous les lieux');fillRestaurantSelect('#restaurantCountry',uniqueRestaurantValues('country'),'Tous les pays');fillRestaurantSelect('#restaurantSpecialty',uniqueRestaurantValues('specialties'),'Toutes les spécialités');}
...
async function initProduce(){
 try{
  ...
  fillProduceSelect('#produceCategory',produceUnique('category'),'Toutes les catégories');
  fillProduceSelect('#produceSeason',produceUnique('season'),'Toutes les saisons');
  fillProduceSelect('#produceAvailability',produceUnique('availability'),'Toutes les disponibilités');
  renderProduce();
