...
function renderRecipes(){const q=norm($('#search').value),cat=$('#category').value,period=$('#recipePeriodFilter')?.value||'';const found=recipes.filter(r=>(!cat||norm(r.category).includes(norm(cat)))&&(!period||(r.period||'indifferente')===period)&&(!q||norm(JSON.stringify(r)).includes(q))).sort((a,b)=>a.title.localeCompare(b.title,'fr',{sensitivity:'base'}));$('#recipeCount').textContent=`${found.length} recette${found.length>1?'s':''}`;$('#recipeList').innerHTML=found.map(recipeCard).join('')||'<p>Aucune recette trouvée.</p>';bindRecipeCards($('#recipeList'));}
...
$('#search').oninput=renderRecipes;$('#category').oninput=renderRecipes;if($('#recipePeriodFilter'))$('#recipePeriodFilter').onchange=renderRecipes;
...
function renderStoreSubgroups(items){const store=items[0]?.store||'',sorted=[...items].sort((a,b)=>(aisleRank(store,a.aisle||'Rayon à définir')-aisleRank(store,b.aisle||'Rayon à définir'))||(a.aisle||'zzz').localeCompare(b.aisle||'zzz','fr',{sensitivity:'base'})||a.name.localeCompare(b.name,'fr',{sensitivity:'base'}));let lastAisle=null,band=-1;return sorted.map(x=>{const aisle=x.aisle||'Rayon à définir',isNew=aisle!==lastAisle;if(isNew){band++;lastAisle=aisle}const bg=band%2?'#e1eee4':'#fffdf8',sep=isNew&&band>0?'border-top:3px solid #9db7a3;':'';return shoppingRow(x,'',`background:${bg};${sep}`)}).join('')}
