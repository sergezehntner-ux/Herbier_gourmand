...
function renderStoreSubgroups(items){const store=items[0]?.store||'';const sorted=[...items].sort((a,b)=>(aisleRank(store,a.aisle||'Rayon à définir')-aisleRank(store,b.aisle||'Rayon à définir'))||(a.aisle||'zzz').localeCompare(b.aisle||'zzz','fr',{sensitivity:'base'})||a.name.localeCompare(b.name,'fr',{sensitivity:'base'}));const aisles=[];sorted.forEach(x=>{const aisle=x.aisle||'Rayon à définir';let group=aisles[aisles.length-1];if(!group||group.aisle!==aisle){group={aisle,items:[]};aisles.push(group)}group.items.push(x)});return aisles.map(group=>`<div class="shop-aisle-band" data-aisle="${esc(group.aisle)}">${group.items.map(shoppingRow).join('')}</div>`).join('')}
