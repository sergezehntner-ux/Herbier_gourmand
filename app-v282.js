...
async function init(){
  ...
  if('serviceWorker' in navigator) navigator.serviceWorker.register('sw.js?v=2950', {updateViaCache:'none'});
...
function renderRecipes(){const q=norm($('#search').value),cat=$('#category').value,max=+$('#maxTime').value||999,tagQ=norm($('#recipeTagSearch')?.value),excludeQ=norm($('#recipeExclude')?.value);const found=recipes.filter(r=>{const tags=(r.tags||[]).map(norm),ingredientText=norm((r.ingredients||[]).map(i=>Array.isArray(i)?i[0]:i).join(' '));return (!cat||r.category===cat)&&(r.time||0)<=max&&(!q||norm(JSON.stringify(r)).includes(q))&&(!tagQ||tags.some(t=>t.includes(tagQ)))&&(!excludeQ||!ingredientText.includes(excludeQ))}).sort((a,b)=>a.title.localeCompare(b.title,'fr',{sensitivity:'base'}));$('#recipeCount').textContent=`${found.length} recette${found.length>1?'s':''}`;$('#recipeList').innerHTML=found.map(recipeCard).join('')||'<p>Aucune recette trouvée.</p>';bindRecipeCards($('#recipeList'));}
...
$('#search').oninput=renderRecipes;$('#category').onchange=renderRecipes;$('#maxTime').onchange=renderRecipes;$('#recipeTagSearch').oninput=renderRecipes;$('#recipeExclude').oninput=renderRecipes;
...
function shoppingRow(x,showAisle=false){const amount=x.qty?`${Math.round(Number(x.qty)*100)/100} ${esc(x.unit)}`:x.text?`${esc(x.text)} ${esc(x.unit)}`:'';return `<div class="shop-item ${x.checked?'checked':''}" data-shopping-id="${esc(x.id)}"><input type="checkbox" data-shop-check="${esc(x.id)}" ${x.checked?'checked':''}><button class="shop-text" data-edit-shopping="${esc(x.id)}"><strong>${esc(x.name)}</strong>${amount?` <span>— ${amount}</span>`:''}${showAisle&&x.aisle?`<small class="shop-aisle">${esc(x.aisle)}</small>`:''}</button></div>`}
function renderStoreSubgroups(items){const store=items[0]?.store||'';return [...items].sort((a,b)=>(aisleRank(store,a.aisle)-aisleRank(store,b.aisle))||a.name.localeCompare(b.name,'fr',{sensitivity:'base'})).map(x=>shoppingRow(x,true)).join('')}
...
$('#restaurantForm').onsubmit=e=>{e.preventDefault();const r=restaurantFromForm();const i=restaurants.findIndex(x=>x.id===r.id);if(i>=0)restaurants[i]=r;else restaurants.unshift(r);const returnScroll=viewScrollPositions.restaurants;saveRestaurants();renderRestaurantFilters();renderRestaurants();viewScrollPositions.restaurants=returnScroll;$('#restaurantDialog').close();if(viewedRestaurantId===r.id)showRestaurant(r.id);};
