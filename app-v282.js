...
async function init(){
  ...
  if('serviceWorker' in navigator) navigator.serviceWorker.register('sw.js?v=29713', {updateViaCache:'none'});
...
let plannerSessionDirty=false,shoppingSessionDirty=false,leaveGuardTarget=null;
...
function requestMainView(id){
  ...
  if(!sessionNeedsBackup(from)){selectionContext=null;updateSelectionBar();openMainView(id);return}
  leaveGuardTarget=id;
...
function finishGuardedLeave(saveDone=false){const from=activeViewId();resetSessionDirty(from);const target=leaveGuardTarget;leaveGuardTarget=null;$('#leaveGuardDialog').close();if(target){selectionContext=null;updateSelectionBar();openMainView(target)}}
let wakeLock=null;
...
function openRecipe(id){if(id)captureRecipeListState();const r=recipes.find(x=>x.id===id);$('#recipeDialogTitle').textContent=r?'Modifier la recette':'Nouvelle recette';$('#recipeId').value=r?.id||'';$('#recipeTitle').value=r?.title||'';$('#recipeCategory').value=r?.category||'';fillRecipeTagSuggestions();$('#recipeTags').value=(r?.tags||[]).join(', ');$('#recipeTime').value=r?.time||30;$('#recipeServings').value=r?.servings||4;$('#recipeTemperature').value=r?.temperature||'chaud';$('#recipeMeat').checked=!!r?.avecViande;$('#recipeIngredients').value=(r?.ingredients||[]).map(i=>i.join(' / ')).join('\n');$('#recipeSteps').value=(r?.steps||[]).join('\n');$('#deleteRecipe').classList.toggle('hidden',!r);$('#duplicateRecipe').classList.toggle('hidden',!r);$('#recipeDialog').showModal();}
...
function shoppingMeasure(qty,unit){const q=Number(qty),u=norm(unit).replace(/\./g,'');if(!Number.isFinite(q))return{qty,unit:unit||'',key:norm(unit)};if(['kg','kilo','kilos','kilogramme','kilogrammes'].includes(u))return{qty:q*1000,unit:'g',key:'g'};if(['g','gr','gramme','grammes'].includes(u))return{qty:q,unit:'g',key:'g'};if(['l','litre','litres'].includes(u))return{qty:q*1000,unit:'ml',key:'ml'};if(['ml','millilitre','millilitres'].includes(u))return{qty:q,unit:'ml',key:'ml'};if(['piece','pieces','pièce','pièces','pc','pcs','unite','unites','unité','unités'].includes(u))return{qty:q,unit:'pièce',key:'piece'};return{qty:q,unit:unit||'',key:u}}
function sameShoppingArticle(a,b){const am=shoppingMeasure(a.qty,a.unit),bm=shoppingMeasure(b.qty,b.unit);return norm(a.name)===norm(b.name)&&am.key===bm.key}
...
function consolidateShopping(){const out=[];shopping.map(normalizeShoppingItem).forEach(x=>{if(x.manual){out.push(x);return}const xm=shoppingMeasure(x.qty,x.unit),probe={...x,qty:xm.qty,unit:xm.unit};const hit=out.find(y=>!y.manual&&sameShoppingArticle(y,probe));if(!hit){out.push(probe);return}const hm=shoppingMeasure(hit.qty,hit.unit);hit.qty=Math.round((Number(hm.qty||0)+Number(xm.qty||0))*100)/100;hit.unit=xm.unit||hit.unit;hit.origins=[...new Set([...(hit.origins||[]),...(x.origins||[])])];hit.originRefs=[...(hit.originRefs||[]),...(x.originRefs||[])].filter((v,i,a)=>a.findIndex(z=>z.recipeId===v.recipeId&&z.date===v.date&&z.slot===v.slot)===i);if(!hit.store&&x.store)hit.store=x.store;if(!hit.aisle&&x.aisle)hit.aisle=x.aisle});shopping=out}
...
function transferMealToShopping(date,slot){const items=mealItems(date,slot);if(!items.length)return alert('Ajoute d’abord une recette à ce repas.');if(!mealPeople(date,slot))return alert('Indique d’abord le nombre de personnes pour ce repas.');const key=mealKey(date,slot),transfers=mealTransfers(),previous=transfers[key]||[];
  previous.forEach(old=>{const om=shoppingMeasure(old.qty,old.unit),x=shopping.find(s=>!s.manual&&sameShoppingArticle(s,{name:old.name,qty:om.qty,unit:om.unit}));if(x&&typeof x.qty==='number'){const xm=shoppingMeasure(x.qty,x.unit);x.qty=Math.max(0,Math.round((Number(xm.qty||0)-Number(om.qty||0))*100)/100);x.unit=xm.unit}});
  shopping=shopping.filter(x=>x.manual||Number(x.qty)>0||x.text);
  const added=[];const prefs=shoppingAssignments();
  items.filter(m=>!m.isLeftover).forEach(m=>{const rows=scaledIngredientRows(m.recipe,m.people,m.qtyOverrides||{});rows.forEach(row=>{const rawq=Number(row.qty);if(!Number.isFinite(rawq)||rawq===0)return;const rm=shoppingMeasure(rawq,row.unit),q=Number(rm.qty);let x=shopping.find(s=>!s.manual&&sameShoppingArticle(s,{name:row.name,qty:q,unit:rm.unit}));const pref=prefs[norm(row.name)]||{};if(!x){x=normalizeShoppingItem({name:row.name,qty:0,unit:rm.unit,store:pref.store||'',aisle:pref.aisle||'',origins:[],originRefs:[]});shopping.push(x)}else{const xm=shoppingMeasure(x.qty,x.unit);x.qty=xm.qty;x.unit=xm.unit}x.qty=Math.round((Number(x.qty||0)+q)*100)/100;const origin=`${dateLabel(date)} ${slot} · ${m.recipe.title}`;if(!x.origins.includes(origin))x.origins.push(origin);x.originRefs=x.originRefs||[];if(!x.originRefs.some(o=>o.recipeId===m.recipe.id&&o.date===date&&o.slot===slot))x.originRefs.push({recipeId:m.recipe.id,title:m.recipe.title,date,slot});added.push({name:row.name,unit:rm.unit,qty:q})})});
  transfers[key]=added;saveMealTransfers(transfers);saveShopping();renderShopping();$('#planFreshness').textContent=`${dateLabel(date)} ${slot.toLowerCase()} transféré dans la liste des courses.`;$('#planFreshness').classList.remove('hidden');shoppingSessionDirty=true;switchView('shopping');
}
